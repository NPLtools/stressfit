...
------------------------------------------------------------------------
 ...
 NOTE: Use DSE, beta, zeta as arrays to calculate multiple shift functions
 simultaneously.
...
"""
import numpy as np
from math import exp, erf, sqrt, pi, log
...
def F0( u, d ):
    return sqrt(pi)/2*(erf(u)-erf(u-d))

def F1( u, d ):
    return 0.5*(exp(-u**2)-exp(-(u-d)**2)) + u*F0(u,d)

def F2( u, d ):
    return 0.5*F0(u,d) + u*F1(u,d) - d*0.5*exp(-(u-d)**2) 
 
def F3( u, d ):
    return 0.5*u*F0(u,d) + (u**2+1)*F1(u,d) - 0.5*d*(d+u)*exp(-(u-d)**2) 
...
class PSModel():
    ...
    def get_ctr(self,h,beta,zeta):
        """ Gauge centre as a function of scan depth (h>0 is under front surface)
    ...
    def get_ctr_ave(self,h,beta,zeta,p):
        """Average of get_ctr function of scan depth (h>0 is under front surface)
    ...
    def get_gwidth(self,h,beta,zeta):
        """ Gauge width as a function of scan depth (h>0 is under front surface)
    ...
    def get_gwidth_ave(self,h,beta,zeta,p):
        """Average of get_gwidth function of scan depth (h>0 is under front surface)
